...
class TestServiceHandler:
    ...
    @pytest.fixture
    def bag_ligplaatsen_data(self, database) -> None:
        data = """
        ...
        data = "\n".join([d.strip() for d in data.splitlines() if d.strip()])
        f = StringIO(data)
        reader = csv.reader(f, delimiter=',')
        for row in reader:
            database.execute(
                f"INSERT INTO bag_ligplaatsen (id, gemeente, object_id, last_update, object) "
                f"VALUES ('{row[0]}', '{row[1]}', '{row[2]}', '{row[3]}', '{row[4]}')"
            )
        database.commit()

    def test_handle_import_msg(self, gob_logger_manager_mock, bag_ligplaatsen_data):
        msg = {
            "header": {
                "catalogue": "bag",
