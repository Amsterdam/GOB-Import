...
class TestValidator(unittest.TestCase):
    ...
    def test_validat_data(self):
        validator = Validator(self.mock_import_client, 'meetbouten', self.valid_meetbouten, 'meetboutidentificatie')
        validator.validate()
    ...
    def test_invalid_data(self):
        validator = Validator(self.mock_import_client, 'meetbouten', self.invalid_meetbouten, 'meetboutidentificatie')
        validator.validate()
    ...
    def test_fatal_value(self):
        validator = Validator(self.mock_import_client, 'meetbouten', self.fatal_meetbouten, 'meetboutidentificatie')

        with self.assertRaises(GOBException):
            validator.validate()

        # Make sure the meetboutidentificatie has been listed as invalid
        self.assertEqual(validator.collection_qa['num_invalid_meetboutidentificatie'], 1)
