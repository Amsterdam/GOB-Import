...
class GebiedenValidator:
    ...
    def __init__(self, catalog_name, entity_name, source_id=None):
        """Initialise GebiedenValidator."""
        self.validate_entity = {"buurten": self.validate_buurt, "bouwblokken": self.validate_bouwblok}.get(entity_name)
        self.source_id = source_id
        self.validated = True

    def result(self):
        """Return GebiedenValidator result."""
        return self.validated

    def validate(self, entity, **kwargs):
        """Validate entity."""
        if self.validate_entity:
